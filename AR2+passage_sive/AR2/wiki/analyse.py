...
sys.path.append('./')
sys.path.append('../')
os.chdir('/home/student2020/wsq/sievedpr/exp')
import random
...
from dpr import BiEncoder,BiEncoder2
...
def get_arguments():
    ...
    parser.add_argument(
        "--model_path",
        default='/webdav/MyData/dpr-cos/dpr_biencoder.29',
        type=str,
    ...
    parser.add_argument(
        "--path_to_dataset",
        default='/home/student2020/wsq/DPR-main/downloads/data/retriever/nq-train.json',
        type=str,
...
def set_env(args):
    ...
    if args.local_rank == -1 or not args.has_cuda:
        device = torch.device("cuda" if torch.cuda.is_available() and args.has_cuda else "cpu")
...
def main_work():
    ...
    train_dataset =TraditionDataset(file_path=args.path_to_dataset,tokenizer=tokenizer,num_hard_negatives=args.num_hard_negatives,
                                    shuffle_positives=True,max_seq_length=512
                                    )
    train_dataloader = DataLoader(train_dataset, collate_fn=TraditionDataset.get_collate_fn(args.num_hard_negatives),batch_size=args.batch_size, shuffle=False)
    # if args.valid:
    ...
    global_step=0
    loss_total=np.zeros((train_dataset.num_queries,args.batch_size*(args.num_hard_negatives+1)))
    #global_step=0
    for epoch in range(num_epochs):
        ...
        for index, sample in enumerate(tqdm(train_dataloader)):
            ...
            if query_num==args.batch_size:
                loss_total[cur_index:cur_index+query_num,:]=sieve_score.reshape((query_num,query_num*(args.num_hard_negatives+1)))
            loss.backward()
            whole_num += 1
            whole_loss += loss
        ...
        pt_save_directory = save_base_directory + args.exp_name+ str(args.global_step)
        args.global_step+=1
        ...
        if args.local_rank in [-1,0]:
            ...
            np.save(pt_save_directory + '/probs',sieve_score_neg)
    ...
    if args.renew:
        ...
        train_dataloader = DataLoader(train_dataset, collate_fn=TraditionDataset.get_collate_fn(args.num_negatives_eval),batch_size=args.batch_size, shuffle=False)
    ...
    cur_index=0
    model.eval()
    sim_total=np.ones((train_dataset.num_queries,args.batch_size*(num_negatives_eval+1)))
    hn_index=np.zeros((train_dataset.num_queries,num_negatives_eval))
    prob_total=np.zeros((train_dataset.num_queries,args.batch_size*(num_negatives_eval+1)))
    label_total=np.zeros((train_dataset.num_queries,args.batch_size*(num_negatives_eval+1)))
    with torch.no_grad():
        for index, sample in enumerate(tqdm(train_dataloader)):
            raw_index+=sample['index']
            query_num=len(sample['query'][0])
            ...
            batch = {'ctx': ctx, 'qry': qry, 'label': label}
            sim, probs,labels = get_model_obj(model).evaluate(batch,ctx_ids.shape,mode=args.mode)
            if args.batch_size==query_num:
                sim_total[cur_index:cur_index+query_num,:]=sim
                prob_total[cur_index:cur_index+query_num,:]=probs
                label_total[cur_index:cur_index+query_num,:]=labels
            
           
            global_step+=1
            cur_index+=query_num
        pt_save_directory = save_base_directory +args.exp_name+ str(args.global_step)+'final'
        loss_v_neg=loss_v_total
        if args.local_rank in [-1,0]:
            if not os.path.exists(pt_save_directory): #判断所在目录下是否有该文件名的文件夹
                os.mkdir(pt_save_directory)
            np.save(pt_save_directory + '/loss-dp',sim_total)
            np.save(pt_save_directory + '/loss-prob',prob_total)
            np.save(pt_save_directory + '/label',label_total)
            np.save(pt_save_directory + '/hnIndex',hn_index)
            np.save(pt_save_directory + '/rawIndex',raw_index)
        if args.local_rank != -1:
            dist.barrier()
