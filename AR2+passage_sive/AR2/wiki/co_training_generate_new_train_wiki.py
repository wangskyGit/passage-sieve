...
def print_gpu_memory_usage():
    ...
    gpu_memory_usage = [int(line.split()[0]) / 1024 for line in output]  # 转换为GB
    num_gpus = len(gpu_memory_usage)

    for i in range(num_gpus):
        print(f"GPU {i} Memory Usage: {gpu_memory_usage[i]:.2f}GB")
...
def reform_out(result_dict_list, q_pos_dict):
    ...
    for infer_result in tqdm(result_dict_list):
        ...
        ### note:here! negative identification during co-training
...
class RenewTools:
    ...
    def get_passage_embedding(self, args, model):
        ...
        if args.load_cache:
            ...
            if args.encode_passages:
                ...
                if args.rank == args.world_size - 1:
                    end_idx = len(self.passages)
                passages = self.passages[start_idx:end_idx]
                logger.info(f'Embedding generation for {len(passages)} passages from idx {start_idx} to {end_idx}')
                
                piece_num = 4
                piece_len = len(passages) // piece_num
                for i in range(piece_num):
                    piece_start_id = i * piece_len
                    piece_end_id = piece_start_id + piece_len
                    if i == piece_num - 1:
                        piece_end_id = len(passages)
                    piece_passages = passages[piece_start_id: piece_end_id]
                    piece_id, piece_embeddings = embed_passages(args, piece_passages, model, self.tokenizer)

                    pickle_path = os.path.join(self.temp_dir,
                                            "{1}_data_obj_{0}_{2}.pb".format(str(args.rank), 'psg_embed', i))
                    with open(pickle_path, 'wb') as handle:
                        pickle.dump(piece_embeddings, handle, protocol=4)

                    pickle_path = os.path.join(self.temp_dir,
                                            "{1}_data_obj_{0}_{2}.pb".format(str(args.rank), 'psg_embed_id', i))
                    with open(pickle_path, 'wb') as handle:
                        pickle.dump(piece_id, handle, protocol=4)
                    logger.info(f'Total passages processed {len(piece_id)}. Written to {pickle_path}.')
                    del piece_id
                    del piece_embeddings
        # dist.barrier()
            else:
                # logger.info('load_passage_begin')
                # passage_embedding_list = []
                # passage_embedding_id_list = []
                # for i in tqdm(range(args.n_gpu)):  # TODO: dynamically find the max instead of HardCode
                ...
                for i in tqdm(range(args.n_gpu)):  # TODO: dynamically find the max instead of HardCode
                    for j in range(piece_num):
                        pickle_path = os.path.join(self.temp_dir, "{1}_data_obj_{0}_{2}.pb".format(str(i), 'psg_embed', j))
                        with open(pickle_path, 'rb') as handle:
                            b = pickle.load(handle)
                            passage_embedding_list.append(b)
                ...
                for i in tqdm(range(args.n_gpu)):  # TODO: dynamically find the max instead of HardCode
                    for j in range(piece_num):
                        pickle_path = os.path.join(self.temp_dir, "{1}_data_obj_{0}_{2}.pb".format(str(i), 'psg_embed_id', j))
                        with open(pickle_path, 'rb') as handle:
                            b = pickle.load(handle)
                            passage_embedding_id_list.append(b)
    ...
    def get_new_faiss_index(self, args, passage_embedding, passage_embedding2id):
        ...
        # gpu_device = faiss.StandardGpuResources()
        # device_id = [1,2,3]  # 指定要使用的 GPU 设备编号，可以是单个值或整数列表
        # resources = faiss.GpuResourcesForDevice(device_id)
    ...
    # given q_a path ,generate, note:here!
    def get_question_topk(self, train_questions,train_answers,
        ...
        if mode == 'test':
            ...
            output_path = os.path.join(self.output_dir, mode + '_ce' + '.json')
            with open(output_path, 'w') as f:
