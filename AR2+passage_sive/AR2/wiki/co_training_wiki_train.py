...
def train(args, model, reranker_model, tokenizer, global_step=0):
    ...
    if global_step != 0:
        train_data_path = os.path.join(args.ann_dir, 'train_ce' + '.json')
        model_path = os.path.join(args.output_dir, 'checkpoint')
        reranker_model_path = os.path.join(args.output_dir, 'checkpoint-reranker')
...
def _save_checkpoint(args, model, optimizer, scheduler, step: int) -> str:
    ...
    cp = os.path.join(args.output_dir, 'checkpoint')
...
def _save_reranker_checkpoint(args, model, optimizer, scheduler, step: int) -> str:
    ...
    epoch = 0
    model_to_save = get_model_obj(model)
    cp = os.path.join(args.output_dir, 'checkpoint-reranker')
...
def get_arguments():
    ...
    parser.add_argument("--a", type=float, default=0.5, help="For a in SimRAS.")
    parser.add_argument("--b", type=float, default=0, help="For b in SimRAS.")
