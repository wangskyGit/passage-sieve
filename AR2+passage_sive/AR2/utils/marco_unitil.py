...
class Rocketqa_v2Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        neg_ids_list = [pair[0] for pair in neg_pairs_list]
    ...
    #     ctx_ids = torch.LongTensor([ctx_token_id + [self.tokenizer.pad_token_id]*(128-len(ctx_token_id)) for ctx_token_id in ctx_token_ids])
    #     c_e_token_ids = torch.LongTensor([temp + [self.tokenizer.pad_token_id]*(160-len(temp)) for temp in c_e_token_ids])
