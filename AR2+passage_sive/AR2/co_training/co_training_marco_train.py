...
from utils.marco_unitil import (
    Rocketqa_v2Dataset,
)
import collections
...
def train(args, model, teacher_model, tokenizer, global_step=0):
    ...
    if global_step!=0:
        train_data_path = os.path.join(args.ann_dir, 'train_ce' + '.tsv')
        
        model_path = os.path.join(args.output_dir, 'checkpoint')
        teacher_model_path = os.path.join(args.output_dir, 'checkpoint-reranker')
